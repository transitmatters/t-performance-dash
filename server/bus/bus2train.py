...
def to_disk(output_by_stop_and_day, root):
  for (stop_id, day), rows in output_by_stop_and_day.items():
    ...
    destination_dir = pathlib.Path(root, "Events", "daily-data", str(stop_id), f"Year={day.year}", f"Month={day.month}", f"Day={day.day}")
...
def main():
  ...
  try:
    with open(availability_path, "r") as file:
      ...
      availability_metadata = merge_availability_dicts(availability_metadata, from_disk)
