...
def peak_off_peak(df):
    ...
    conditions  = [(df['holiday']==False) & (df['weekday'] < 5) & ((df['dep_time']>=datetime.time(6,30,0)) & (df['dep_time']<datetime.time(9,0,0))), (df['holiday']==False) & (df['weekday'] < 5) & ((df['dep_time']>=datetime.time(15,30,0)) & (df['dep_time']<datetime.time(18,30,0)))]
...
def travel_times_over_time(sdate, edate, params):
    ...
    # get summary stats
    summary_stats      = df.groupby('service_date')['travel_time_sec'].describe() #summary stats for all
    summary_stats['peak'] = 'all'
    summary_stats = summary_stats.reset_index() # reset_index
    summary_stats_peak = df.groupby(['service_date', 'peak'])['travel_time_sec'].describe().reset_index() #summary_stats for peak / off-peak trains
