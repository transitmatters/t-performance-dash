...
export class StationConfiguration extends React.Component {
  ...
  optionsForField(type) {
    ...
    if (type === 'from') {
      ...
      return options_station_ui(this.props.current.line).filter(
        (entry) => entry.value !== toStation
      );
    }
    if (type === 'to') {
  ...
  render() {
    ...
    return (
      <div className={classNames('station-configuration-wrapper', bus_mode ? 'Bus' : currentLine)}>
        <div className="station-configuration main-column">
          ...
          <button
            ...
            onClick={this.handleSwapStations}
